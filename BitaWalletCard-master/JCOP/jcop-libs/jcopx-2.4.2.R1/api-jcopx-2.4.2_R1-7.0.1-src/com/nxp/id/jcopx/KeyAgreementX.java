...
public final class KeyAgreementX {
	/**
	 * KeyAgreement algorithm ALG_EC_SVDP_DH_PLAIN_XY is the same as ALG_EC_SVDP_DH_PLAIN
	 * but but with additional Y component and a leading 0x04, which is the format
	 * of an uncompressed point.
	 */
	 ...
	 *            ALG_EC.. constants above, for example, ALG_EC_SVDP_DH_PLAIN_XY
	 * @param externalAccess
	 *            if true indicates that the instance will be shared among
	 *            multiple applet instances and that the KeyAgreement instance
	 *            will also be accessed (via a Shareable interface) when the
	 *            owner of the KeyAgreement instance is not the currently
	 *            selected applet. If true the implementation must not allocate
	 ...
	 *             <ul>
	 *             <li>CryptoException.NO_SUCH_ALGORITHM if the requested
	 *             algorithm or shared access mode is not supported.</li>
	 *             </ul>
	 */
